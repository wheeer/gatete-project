...
classDiagram
    ...
    note for RazaResource "Campos base. Ver catálogo completo en documentación interna"
...
flowchart TD
    ...
    %% Rama de Habilidades
    B -- HABILIDAD --> C[Aplicar Daño Escalar a Vida/Postura]
    C --> D[Aplicar Estados Especiales/Debuffs]
    
    %% Rama de Parry (Efecto en atacante/enemigo)
    B -- PARRY_JUGADOR --> E[Daño Masivo a Postura + Mini-Stun]
    
    %% Rama de Golpes de Combo
    B -- GOLPE_COMBO --> F{¿Combo Index == 3?}
    
    F -- SÍ: CRÍTICO --> G[GOLPE CRÍTICO: Daño xN + Mini-Stun]
    G --> H[Habilitar Ventana de Caza Temporal]
    
    F -- NO --> I[Daño Normal a Vida/Postura]
    
    %% Resolución de Impacto (Común)
    C & E & H & I --> J[Calcular Mitigación según Raza/ADN]
    J --> K{¿Postura > 0?}
    
    K -- SÍ --> L[Reducir Postura]
    K -- NO --> M[Estado: POSTURE_BROKEN]
    M --> N[Habilitar Intento de CAZA / EJECUCIÓN]
    
    L --> O{¿Vida <= 15%?}
    O -- SÍ --> N
    O -- NO --> P[Disparar EVT_RECIBIR_GOLPE]
    
    %% Capa Mental (Exclusiva Enemigo)
    P --> Q[Actualizar Impulso Psicológico: IRA o PANICO]
    Q --> R{¿Muerte?}
